...
export default function App() {
  ...
  const reminderPlan = data.weeklyPlans[reminderWeek] ?? createWeeklyPlan(reminderWeek)
  ...
  return <>
    ...
    <div className="day-builder">
      ...
      <section className={`schedule-board ${armedTemplateId ? 'placing' : ''}`}>
        <div className="schedule-instructions"><div><span>{editMode ? (language === 'zh' ? '拖动已安排模块可重新排期' : 'Drag scheduled blocks to reschedule') : (language === 'zh' ? '开启编辑模式后可移动已有模块' : 'Turn on edit mode to move scheduled blocks')}</span><small>{language === 'zh' ? '00:00–24:00 · 上下滚动查看全天' : '00:00–24:00 · Scroll to view the full day'}</small></div>{armedTemplateId && <b>{language === 'zh' ? '点击时间轴放置选中的模块' : 'Click the timeline to place the selected module'}</b>}</div>
        <div
...
function WeekView({ data, setData, selectedDate, language, t, conflicts, setSelectedDate, setView, reviewRequested, onReviewOpened }: SharedProps & { setData: React.Dispatch<React.SetStateAction<PlannerData>>; conflicts: Set<string>; setSelectedDate: (date: string) => void; setView: (view: View) => void; reviewRequested: boolean; onReviewOpened: () => void }) {
  ...
  return <>
    <WeeklyDashboard {...{ data, setData, selectedDate, language, reviewRequested, onReviewOpened }} onOpenFloorSettings={() => setView('settings')} />
...
function BlockModal({ open, block, date, data, language, onClose, onSave }: { open: boolean; block?: TimeBlock; date: string; data: PlannerData; language: 'zh' | 'en'; onClose: () => void; onSave: (block: TimeBlock) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={event => event.currentTarget === event.target && onClose()}>
    <form className="modal" onSubmit={submit}><header><div><span className="eyebrow">{block ? (language === 'zh' ? '编辑时间模块' : 'Edit time block') : (language === 'zh' ? '新建时间模块' : 'New time block')}</span><h2>{language === 'zh' ? '安排一段专注时间' : 'Plan a focused block'}</h2></div><button type="button" className="ghost icon" onClick={onClose}><X /></button></header>
      ...
      <div className="completion-row"><label>{language === 'zh' ? '完成状态' : 'Completion'}<select value={form.status ?? 'pending'} onChange={event => { const status = event.target.value as TimeBlock['status']; patch({ status, completedMinutes: status === 'completed' && form.startTime && form.endTime ? duration(form.startTime, form.endTime) : status === 'partial' ? form.completedMinutes ?? 0 : 0 }) }}><option value="pending">{language === 'zh' ? '未开始' : 'Not started'}</option><option value="partial">{language === 'zh' ? '部分完成' : 'Partial'}</option><option value="completed">{language === 'zh' ? '已完成' : 'Completed'}</option><option value="skipped">{language === 'zh' ? '已跳过' : 'Skipped'}</option></select></label>{(form.status === 'partial' || form.status === 'completed') && <label>{language === 'zh' ? '实际完成分钟' : 'Actual minutes'}<span className="minute-stepper"><button type="button" onClick={() => patch({ completedMinutes: Math.max(0, (form.completedMinutes ?? 0) - 15) })}>−15</button><input type="number" min="0" max={form.status === 'partial' && form.startTime && form.endTime ? duration(form.startTime, form.endTime) : 1440} step="15" value={form.completedMinutes ?? 0} onChange={event => patch({ completedMinutes: Number(event.target.value) })} /><button type="button" onClick={() => patch({ completedMinutes: Math.min(form.status === 'partial' && form.startTime && form.endTime ? duration(form.startTime, form.endTime) : 1440, (form.completedMinutes ?? 0) + 15) })}>+15</button></span></label>}</div>
